...
router.put("/:id/meet/attendall", verifyToken, (req, res) => {
  User.findById(decodedId, { password: 0 }, (err, user) => {
    if (err)
      return res.status(500).send("There was a problem finding the user.");
    if (!user) return res.status(404).send("No user found.");
    const member = { name: req.body.name };
    if (req.body.dbmethod == "push" && req.body.attendance == "attending") {
      Community.updateOne(
    ...
    if (req.body.dbmethod == "pull" && req.body.attendance == "attending") {
      Community.updateOne(
        { _id: req.params.id, "meets._id": req.body.meet },
    ...
    if (req.body.dbmethod == "push" && req.body.attendance == "notattending") {
      Community.updateOne(
        { _id: req.params.id, "meets._id": req.body.meet },
    ...
    if (req.body.dbmethod == "pull" && req.body.attendance == "notattending") {
      Community.updateOne(
        { _id: req.params.id, "meets._id": req.body.meet },
    ...
    if (
      req.body.dbmethod == "push" &&
      req.body.attendance == "maybeattending"
    ) {
      Community.updateOne(
        { _id: req.params.id, "meets._id": req.body.meet },
